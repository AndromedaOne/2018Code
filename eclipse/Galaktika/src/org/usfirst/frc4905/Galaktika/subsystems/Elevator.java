...
public class Elevator extends Subsystem {
	...
	private double m_encoderPIDP_maintanence = 0.015;// p constant for maintaining position, way too big for distance
														// traveling
	...
	private class EncoderPIDOut implements PIDOutput {
		...
		public void pidWrite(double output) {
			...
			if (noisyDebug) {
				System.out.println("In Elevator pidWrite output = " + output + " Current error = "
						+ m_encoderPID.getError() + "Current Position: " + getElevatorEncoderPosition());
			}
		}
	}
	...
	public void initializeEncoderPID() {
		...
		m_encoderPID = new PIDController(m_encoderPIDP_maintanence, m_encoderPIDI, m_encoderPIDD, m_encoderPIDF,
				encoderPIDIn, encoderPIDOut);
	...
	public void moveElevatorSafely(double speed) {
		// negative velocities drive the elevator up, positive velocities drive it
		// down...

		if (getTopLimitSwitch() == false && (speed < 0)) {
			// trying to go upwards when speed < 0, false on the limit switch means it is
			// triggered
			...
			moveElevator(0);
			// if the pid loop drives somewhere unsafe, we should probably end the PID loop
			// if its running
	...
	public double getElevatorPosition() {
		...
		return elevatorController.getSelectedSensorPosition(0) - m_encoderZeroPostion;
	}
	...
	public void setPIDControllerToTravelMode() {
		...
		// have a lower tolerance, just want to get close, then we'll switch to
		// maintanence mode and more aggressive correction constants
	}
